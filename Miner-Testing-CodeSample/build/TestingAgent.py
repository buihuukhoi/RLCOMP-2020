...
try:
    ...
    minerEnv.reset()
    state_map, state_users = minerEnv.get_state()  ##Getting an initial state
    while not minerEnv.check_terminate():
        try:
            action = np.argmax(DQNAgent.predict({"state_map": state_map.reshape(1, 21, 9, 7), "state_users": state_users.reshape(1, 60)}))
